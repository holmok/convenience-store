...
class Store {
  constructor (path, { compress = false, password, salt, algorithm = 'AES-256-CBC', cache = 100, serializer = undefined } = {}) {
    this.cache = new CacheManager(cache)
    this.buckets = new BucketManager(path, { compress, password, salt, algorithm, serializer })
  ...
  create (bucket, item) {
    ...
    const { start, length } = this.store.create(item, path, serializer)
  ...
  update (bucket, id, item) {
    ...
    const { start, length } = this.store.update(key, prev, item, path, serializer)
