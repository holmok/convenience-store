...
Tape('store test compress and filter', (t) => {
  ...
  const { items } = store.filterItems(bucket, (i) => i.active, { take: 20 })
  t.equal(items.length, 10, 'Items to delete = 10 items')
  for (const item of items) {
    store.delete(bucket, item.id)
  }
...
Tape('store test filter paging', (t) => {
  ...
  let items1 = store.filterItems(bucket, (i) => i.active, { offset: 0, take: 2 })
  t.equal(items1.more, true, 'got more when only taking 2')
  t.equal(items1.items.length, 2, 'got 2 item on taking 2')

  let items2 = store.filterItems(bucket, (i) => i.active, { offset: 8, take: 2 })
  ...
  let items3 = store.filterItems(bucket, (i) => i.active, { offset: 9, take: 2 })
